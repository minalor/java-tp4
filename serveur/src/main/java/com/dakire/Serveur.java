...
public class Serveur extends UnicastRemoteObject implements com.dakire.MorpionInterface {
    ...
    /**
     * Est initialisé à true, et passe à false si il y a déjà eu une première
     * connexion
     */
    private boolean firstConnection = true;

    /**
     * Est initialisée à false, et sera true quand les deux clients seront
     * connectés.
     ...
     *
     * @param bouton     L'emplacement sur la grille où le joueur veut jouer son
     *                   coup
    ...
    public static void main(String[] args) {
        try {
            ...
            Serveur morpion = new Serveur();
            // Naming.rebind("rmi://localhost/MorpionService", morpion);

            Registry registry = LocateRegistry.createRegistry(2023);
            System.out.println("Created registry");

            // MorpionInterface stub = (MorpionInterface)
            // UnicastRemoteObject.exportObject(morpion, 0);
            // System.out.println("Exported Server");

            // Bind the remote object's stub in the registry
            registry.rebind("MorpionService", morpion);
            System.out.println("Morpion RMI server is ready.");
        } catch (Exception e) {
            System.out.println("Erreur avec rmi lors du lancement: " + e.toString());
            e.printStackTrace();
        }
    }
