...
public class old_GUI extends UnicastRemoteObject implements ActionListener, Callback {
     ...
     */
    old_GUI() throws RemoteException {
        try {
        ...
        inputDialog.setTitle("Nom du joueur");
        inputDialog.setHeaderText("Entrez le nom du joueur");
        inputDialog.setContentText("Nom du joueur :");

        Optional<String> result = inputDialog.showAndWait();
        if (result.isPresent()) {
            nomJoueur = result.get().trim();
        }
        ...
        if (nomJoueur.trim().isEmpty()) {
            ...
            alert.setContentText("Vous devez entrer un nom de joueur valide.");
            alert.showAndWait();
        }
